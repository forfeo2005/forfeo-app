...
// --- DASHBOARD ADMIN (Correction Erreur Capture 1) ---
app.get('/admin/dashboard', async (req, res) => {
    if (req.session.userRole !== 'admin') return res.redirect('/login');
    try {
        const entreprises = await pool.query("SELECT * FROM users WHERE role = 'entreprise'");
        // Utilisation de LEFT JOIN pour ne pas crasher si pas d'entreprise liée
        ...
        console.error(err);
        res.status(500).send("Erreur Admin : " + err.message); 
    }
});

// --- DASHBOARD AMBASSADEUR (Correction Erreur Capture 2) ---
...
// --- DASHBOARD ENTREPRISE (Capture 3 - Ajout Limite Freemium) ---
