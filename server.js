...
app.use(session({
    ...
    secret: 'forfeo_v20_final_ultimate',
...
async function setupDatabase() {
    try {
        ...
        // Migration Colonnes
        await pool.query(`ALTER TABLE users ADD COLUMN IF NOT EXISTS telephone VARCHAR(50);`);
        await pool.query(`ALTER TABLE users ADD COLUMN IF NOT EXISTS adresse TEXT;`);
...
app.get('/admin/dashboard', async (req, res) => {
    ...
    // Calculs Finance (TPS 5% + TVQ 9.975%)
...
const checkLimit = async (req, res, next) => {
    ...
    // SI PRO, ON PASSE
    if (user.rows[0].forfait === 'Pro') return next();
    
    // SI FREEMIUM, CHECK LIMIT
...
app.get('/entreprise/upgrade-success', async (req, res) => {
    ...
    res.redirect('/entreprise/dashboard?msg=welcome_pro');
});
...
app.post('/profil/update', async (req, res) => {
    // Mise à jour complète avec téléphone et adresse
