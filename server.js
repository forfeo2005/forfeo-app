...
// Note: Doit être placé AVANT les middlewares express.json()
...
// --- ROUTES PROFIL (Les deux nouvelles routes) ---
...
app.get('/admin/dashboard', async (req, res) => {
    ...
    try {
        ...
        // Nettoyage SQL pour les totaux (évite crash sur $ ou vide)
...
// --- DASHBOARD AMBASSADEUR (Fix Erreur Numeric) ---
app.get('/ambassadeur/dashboard', async (req, res) => {
    ...
    } catch (err) { console.error(err); res.status(500).send("Erreur Ambassadeur"); }
});
...
app.get('/entreprise/dashboard', async (req, res) => {
    ...
    try {
        ...
        res.render('entreprise-dashboard', { 
            ...
            stats: { totale: missions.rows.length, forfait: forfait, canPublish: (forfait !== 'Freemium' || missions.rows.length < 1) }
        });
...
app.post('/creer-mission', async (req, res) => {
    ...
    try {
        await pool.query("INSERT INTO missions (entreprise_id, titre, description, recompense, type_mission, statut) VALUES ($1, $2, $3, $4, $5, 'actif')", [req.session.userId, titre, description, recompense, type_mission]);
