...
async function setupDatabase() {
    try {
        // Ajout de la colonne manquante identifiée dans les logs Render
        await pool.query(`ALTER TABLE missions ADD COLUMN IF NOT EXISTS type_audit VARCHAR(100) DEFAULT 'Audit Standard';`);
        
        // Ajout de la contrainte d'unicité pour les scores (nécessaire pour ON CONFLICT)
        await pool.query(`
            DO $$ 
            BEGIN 
        ...
        await pool.query(`
            ...
            VALUES (1, 1, 'Qu''est-ce qui définit le harcèlement psychologique ?', 'Conflit simple', 'Conduite vexatoire répétée', 'Critique travail', 'B')
            ON CONFLICT (id) DO NOTHING;
        `);
        console.log("✅ FORFEO LAB : Base de données synchronisée et réparée.");
    } catch (err) { console.error("❌ Erreur de configuration DB:", err); }
}
setupDatabase();
...
app.listen(port, () => console.log(`🚀 FORFEO LIVE SUR PORT ${port}`));
