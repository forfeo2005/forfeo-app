...
async function setupDatabase() {
    try {
        await pool.query(`
            ...
            CREATE TABLE IF NOT EXISTS missions (id SERIAL PRIMARY KEY, entreprise_id INTEGER, ambassadeur_id INTEGER, titre VARCHAR(255), description TEXT, recompense VARCHAR(50), statut VARCHAR(50) DEFAULT 'actif', created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP);
            ...
            SELECT 1, 'Qu''est-ce qui définit le harcèlement psychologique ?', 'Conflit simple', 'Conduite vexatoire répétée', 'Critique travail', 'B' WHERE NOT EXISTS (SELECT 1 FROM formations_questions WHERE id = 1);
        `);
...
// --- PORTAIL ENTREPRISE : AUDITS & EMPLOYÉS ---
...
app.post('/entreprise/creer-audit', async (req, res) => {
    ...
    res.redirect('/entreprise/dashboard?msg=Audit publié');
});
...
app.post('/entreprise/ajouter-employe', async (req, res) => {
    ...
    res.redirect('/entreprise/dashboard?msg=Employé inscrit');
});
